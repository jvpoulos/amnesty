...
response.dat <- data.frame(x = c(paste0("Personal property value in 1870,\n N=",format(as.numeric(nrow(na.omit(slaveholders.60.rd[c("Personal Estate 1870")]))), nsmall=0, big.mark=",")),
                                 ...
                                 paste0("Change in personal property value, 1860-1870,\n N=",format(as.numeric(nrow(na.omit(slaveholders.60.rd[c("persprop.d")]))), nsmall=0, big.mark=",")),
                                 paste0("Change in real estate value, 1860-1870,\n N=",format(as.numeric(nrow(na.omit(slaveholders.60.rd[c("realprop.d")]))), nsmall=0, big.mark=",")),
                                 paste0("Change in total census wealth, 1860-1870,\n N=",format(as.numeric(nrow(na.omit(slaveholders.60.rd[c("taxprop.d")]))), nsmall=0, big.mark=","))),
...
ForestPlot(response.dat,
           xlab="Treatment effect estimate",ylab="Census wealth (1860$)") +
